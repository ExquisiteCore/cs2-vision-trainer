...
from cs2_vision_trainer.dataset.schema import DEFAULT_CLASS_NAMES, class_name, normalize_class_names
...
class AnnotatorState:
    ...
    def __post_init__(self) -> None:
        self.class_names = normalize_class_names(self.class_names)
    ...
    def current_class_name(self) -> str:
        return class_name(self.class_names, self.class_index)
...
def draw_annotation_overlay(
    ...
    *,
    ...
    for index, box in enumerate(state.boxes or []):
        ...
        color = _class_color(box.class_index)
        cv2.rectangle(output, (x1, y1), (x2, y2), color, 2)
        cv2.putText(
            output,
            f"{class_name(state.class_names, box.class_index)} {index + 1}",
            (x1, max(20, y1 - 6)),
    ...
    if state.drawing_start and state.drawing_current:
        ...
        cv2.rectangle(output, preview[:2], preview[2:], _class_color(state.class_index), 2)
...
def _draw_annotation_help(output: np.ndarray, *, state: AnnotatorState) -> None:
    ...
    lines = [
        ...
        f"active {state.class_index + 1}:{state.current_class_name} | 1-4 switch | left draw | right delete | Space save next | Q quit",
    ]
    y = 28
    for line in lines:
        cv2.putText(
    ...
    cv2.putText(
        output,
        "Empty frames are saved as empty labels.",
        (12, max(height - 16, 16)),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.58,
        (255, 255, 255),
